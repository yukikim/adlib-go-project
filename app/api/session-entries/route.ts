...
export async function POST(request: NextRequest) {
  ...
  if (!body?.sessionEventId || !body.attendanceStatus) {
  ...
  if (memberProfile.mainInstrument === 'vocal' && requests.some((item) => !item.keyName?.trim())) {
  ...
  const existingSongs = await prisma.song.findMany({
    where: { title: { in: requests.map((item) => item.songTitle.trim()) } },
  ...
  const entry = await prisma.$transaction(async (tx) => {
    ...
    for (const item of requests.sort((a, b) => a.priority - b.priority)) {
      ...
      if (!song && item.round === 1) {
        song = await tx.song.create({ data: { title: item.songTitle.trim() } });
      ...
      await tx.sessionEntryRequest.create({
        data: {
          ...
          keyName: item.keyName?.trim() || null,
          round: item.round,
